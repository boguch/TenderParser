...
$params = [
    'sortAsc' => 'false', // Порядок сортировки
...
for ($i = 1076; $i < $recordsToProcess; $i++) {
...
$database = 'Rusala_tender'; // имя базы данных
...
$stmt = $mysqli->prepare("INSERT INTO tender (TenderNumber, OrganizerName, TenderViewUrl, DocumentLinks, DocumentNames, StartDate) VALUES (?, ?, ?, ?, ?, ?)");
...
for ($i = 1076; $i < $recordsToProcess; $i++) {
    ...
    // Привязка параметров
    $stmt->bind_param("ssssss", $tenderNumber, $organizerName, $tenderViewUrl, $documentLinks, $documentNames, $startDate);
